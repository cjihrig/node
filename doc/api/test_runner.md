...
The `test_runner` facilitates the creation of JavaScript tests that report
results in [TAP][] format. To access it:
...
* `fn` {Function} The function under test. The test fails if this function
  throws an exception or returns a `Promise` that rejects. This function is
  invoked with a single [`TestContext`][] argument.
