...
// TODO(cjihrig): Need something to keep the event loop alive artificially.
...
class Test {
  ...
  constructor(testNumber, name, fn, reporter, indent, indentString) {
    ...
    this.subtests = [];
    this.reporter = reporter; // TODO(cjihrig): Unused?
    this.indent = indent;
  ...
  finalize() {
    ...
    if (this.skipped) {
      this.reporter.skip(this.indent, this.testNumber, message);
    } else if (this.passed) {
      this.reporter.ok(this.indent, this.testNumber, message);
    } else {
      this.reporter.fail(this.indent, this.testNumber, message);
    }
