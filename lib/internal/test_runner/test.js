...
class Test extends AsyncResource {
  ...
  // TODO(cjihrig): Add tests for all of these combinations.
  // (), (name), (options), (fn), (name, options), (name, fn), (options, fn), (name, options, fn)
  createSubtest(name, options, fn) {
    ...
    if (parent.waitingOn === 0) {
      ...
      // If the parent is the root test, and this is its first subtest, then
      // this is the very first test in the file. Make sure the event loop
      // stays alive.
      if (parent.parent === null) {
        parent.keepAlive = setInterval(noop, kMaxTimeout);
      }
    }
