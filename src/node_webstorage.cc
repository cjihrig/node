...
bool Storage::Open() {
  ...
  static const char get_user_version_sql[] = "PRAGMA user_version";
  ...
  // Get the current user_version, used to determine schema migrations.
  sqlite3_stmt* s = nullptr;
  r = sqlite3_prepare_v2(db, get_user_version_sql, -1, &s, 0);
  CHECK_ERROR_OR_THROW(env(), r, SQLITE_OK, false);
  ...
  int user_version = sqlite3_column_int(stmt.get(), 0);
  stmt = nullptr;  // Force finalization.

  if (user_version > kCurrentSchemaVersion) {
    node::THROW_ERR_INVALID_STATE(
      env(),
      "localStorage was created with a newer version of Node.js");
    return false;
  }

  if (user_version < kCurrentSchemaVersion) {
    // Add a case for each schema migration. Allow fall-through between cases
    // so that all subsequent migrations run. In the future, we may want a
    // transaction here as well, but that is currently unnecessary. A
    // transaction will also require splitting out the PRAGMAs as some of them
    // cannot happen within a transaction.
    switch (user_version) {
      case 0:
