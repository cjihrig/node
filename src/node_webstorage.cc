...
Storage::~Storage() {
  CHECK_EQ(sqlite3_close(db_), SQLITE_OK);
  db_ = nullptr;
}
...
bool Storage::Open() {
  ...
  int r = sqlite3_open(location_.c_str(), &db_);
  CHECK_ERROR_OR_THROW(env(), r, SQLITE_OK, false);
  r = sqlite3_exec(db_, sql, 0, 0, nullptr);
  CHECK_ERROR_OR_THROW(env(), r, SQLITE_OK, false);
  return true;
}
...
void Storage::Clear() {
  ...
  CHECK_ERROR_OR_THROW(
      env(), sqlite3_prepare_v2(db_, sql, -1, &stmt, 0), SQLITE_OK, void());
