...
set NATIVE_SUITES=addons js-native-api node-api ffi embedding
...
if "%target%"=="TestClean" (
  ...
  echo deleting test\ffi\.buildstamp
  if exist "test\ffi\.buildstamp" del /f /q "test\ffi\.buildstamp"
  echo deleting test\ffi\fixture_library\build
  if exist "test\ffi\fixture_library\build" rmdir /S /Q "test\ffi\fixture_library\build"
  goto exit
)
